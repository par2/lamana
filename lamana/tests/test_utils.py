...
class TestExport():
    ...
    # Setup -------------------------------------------------------------------
    # TODO: Make a Fixture
    # case = distributions.laminator(dft.geos_standard)[0]
    # LM = case.LMs[0]
    # FI = LM.FeatureInput
    L = constructs.Laminate(dft.FeatureInput)
    LM = constructs.LaminateModel(dft.FeatureInput)
    ...
    def test_export_csv_temp1(self):
        ...
        try:
            ...
            data_fpath, dash_fpath = output_.export(
    ...
    def test_export_xlsx_default(self):
        ...
        result = output_.export(self.LM, suffix=None, temp=True, delete=True)
    ...
    def test_export_xlsx_error1(self):
        ...
        output_.export(self.LM, dirpath='dummy', delete=True)
...
class TestFeatureInputTools:
    ...
    def test_tool_convert_featureinput2(self):
        ...
        converted_dict = input_.convert_featureinput(dict_frame)
...
#
#     '''
#     case = distributions.laminator(['400-200-800'])
#     try:
#         ##case = distributions.laminator(['400-200-800'])
...
#
#     '''
#     case = distributions.laminator(['400-200-800', '400-200-800'])
#     filepaths = []
#     try:
#         ##case = distributions.laminator(['400-200-800', '400-200-800'])
#         # Write files to default output dir
...
#     case = distributions.laminator(['400-200-800', '400-[100,100]-800'])
#     written_filepaths = []
...
def test_getmultigeo1():
    ...
    for case_ in case.values():
        for LM in case_.LMs:
            actual = input_.get_multi_geometry(LM.LMFrame)
...
def test_getmultigeo2():
    ...
    case = distributions.laminator(['400-200-0'])
    for case_ in case.values():
        for LM in case_.LMs:
            actual = input_.get_multi_geometry(LM.LMFrame)
