...
class TestBaseModel():
    ...
    class Parent(theories.BaseModel):
        def _use_model_():
            pass
        pass
    ...
    def test_theories_BaseModel_error1():
        ...
        class Model(theories.BaseModel):
            pass
    ...
    def test_theories_BaseModel_subclass1(self):
        ...
        actual1 = issubclass(self.Parent, theories.BaseModel)
        actual2 = issubclass(self.Child, self.Parent)
        actual3 = issubclass(self.Child, theories.BaseModel)
        actual4 = isinstance(self.parent, theories.BaseModel)
        actual5 = isinstance(self.child, self.Parent)
        actual6 = isinstance(self.child, theories.BaseModel)
        nt.assert_true(actual1)
        nt.assert_true(actual2)
...
def test_theories_FeatureInput_globels2():
    ...
    case = distributions.laminator(geos=dft.geos_standard, ps=[1])
    for case_ in case.values():
        for LM in case_.LMs:
...
def test_theories_matl_order1():
    ...
    case = distributions.laminator(geos=dft.geos_full)
    expected_mix = [
        ['HA'],
        ['HA', 'PSu'],
        ['HA', 'PSu', 'HA'],
...
class TestHandshake:
    ...
    def test_theories_handshake_hookclass1(self):
        ...
        nt.assert_is_instance(case, distributions.Case)
